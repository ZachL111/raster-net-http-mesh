...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-raster-net-http-detail.ps1

node dist/tests/domainReview.test.js

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-raster-net-http-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-raster-net-http-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-raster_net_http_mesh-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
